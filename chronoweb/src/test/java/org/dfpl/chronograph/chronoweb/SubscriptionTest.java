...
public class SubscriptionTest {
	...
	public void testOutIsAfterReachability() throws IOException {
		JsonObject expectedResult = new JsonObject().put("time", 0).put("source", SOURCE_VERTEX_ID)
				.put("program", KAIROS_PROGRAM_NAME).put("edgeLabel", EDGE_LABEL);

		// 1. Subscribe time: 0, kairosProgram: OutIsAfterReachability, edgeLabel:
		// label, source vertex: a
		String subsribeURL = String.format("%s/graph/%s/OutIsAfterReachability/%s/%s", HOST, SOURCE_TIME, EDGE_LABEL,
				SOURCE_VERTEX_ID);
		...
		// 3. GET time: 0, kairosProgram: OutIsAfterReachability, edgeLabel: label,
		// source vertex: a
		...
		// 5. GET time: 0, kairosProgram: OutIsAfterReachability, edgeLabel: label,
		// source vertex: a
		...
		// 6. PUT c|label|d_3
		postEdgeEvent("c", "d", "3");
		// 7. GET time: 0, kairosProgram: OutIsAfterReachability, edgeLabel: label,
		// source vertex: a
		expectedResult.put("gamma", new JsonObject().put("a", 0).put("b", 1).put("c", 5));
		getReachability(expectedResult);
		...
		// 8. PUT b|label|c_3
		postEdgeEvent("b", "c", "3");
		// 9. GET time: 0, kairosProgram: OutIsAfterReachability, edgeLabel: label,
		// source vertex: a
		...
		delEdgeEvent("b", "c", "3");
		// 11. GET time: 0, kairosProgram: OutIsAfterReachability, edgeLabel: label,
		// source vertex: a
		expectedResult.put("gamma", new JsonObject().put("a", 0).put("b", 1).put("c", 5));
		getReachability(expectedResult);
	}
	...
	private void getReachability(JsonObject expectedResult) throws IOException {
		String getURLString = String.format("%s/gammaTable/%s/OutIsAfterReachability/%s/%s", HOST, SOURCE_TIME,
				EDGE_LABEL, SOURCE_VERTEX_ID);
